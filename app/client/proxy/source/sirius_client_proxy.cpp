...
int32_t sirius::app::client::proxy::handler::seek_to(int32_t second)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_proxy)
		status = _proxy->seek_to(second);

	return status;
}

int32_t sirius::app::client::proxy::handler::key_up_play_toggle(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_proxy)
		status = _proxy->key_up_play_toggle();

	return status;
}

int32_t sirius::app::client::proxy::handler::key_up_backward(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_proxy)
		status = _proxy->key_up_backward();

	return status;
}

int32_t sirius::app::client::proxy::handler::key_up_forward(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_proxy)
		status = _proxy->key_up_forward();

	return status;
}

int32_t sirius::app::client::proxy::handler::key_up_reverse(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_proxy)
		status = _proxy->key_up_reverse();

	return status;
}

int32_t sirius::app::client::proxy::handler::key_down_seek(int32_t diff)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_proxy)
		status = _proxy->key_down_seek(diff);

	return status;
}

int32_t sirius::app::client::proxy::handler::key_up_seek(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_proxy)
		status = _proxy->key_up_seek();
...
int32_t sirius::app::client::proxy::seek_to(int32_t second)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_core)
		status = _core->seek_to(second);

	return status;
}

int32_t sirius::app::client::proxy::key_up_play_toggle(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_core)
		status = _core->key_up_play_toggle();

	return status;
}

int32_t sirius::app::client::proxy::key_up_backward(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_core)
		status = _core->key_up_backward();

	return status;
}

int32_t sirius::app::client::proxy::key_up_forward(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_core)
		status = _core->key_up_forward();

	return status;
}

int32_t sirius::app::client::proxy::key_up_reverse(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_core)
		status = _core->key_up_reverse();

	return status;
}

int32_t	sirius::app::client::proxy::key_down_seek(int32_t diff)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_core)
		status = _core->key_down_seek(diff);

	return status;
}

int32_t	sirius::app::client::proxy::key_up_seek(void)
{
	int32_t status = sirius::app::client::proxy::err_code_t::fail;

	if (_core)
		status = _core->key_up_seek();
...
void sirius::app::client::proxy::on_pre_playback_end(void)
{
	if (_handler)
		_handler->on_pre_playback_end();
}

void sirius::app::client::proxy::on_playback_end(void)
{
	if (_handler)
		_handler->on_playback_end();
}

void sirius::app::client::proxy::on_post_playback_end(void)
{
	if (_handler)
		_handler->on_post_playback_end();
}

void sirius::app::client::proxy::on_pre_playback_totaltime(int32_t tottime)
{
	if (_handler)
		_handler->on_pre_playback_totaltime(tottime);
}

void sirius::app::client::proxy::on_playback_totaltime(int32_t tottime)
{
	if (_handler)
		_handler->on_playback_totaltime(tottime);
}

void sirius::app::client::proxy::on_post_playback_totaltime(int32_t tottime)
{
	if (_handler)
		_handler->on_post_playback_totaltime(tottime);
}

void sirius::app::client::proxy::on_pre_playback_currenttime(int32_t curtime)
{
	if (_handler)
		_handler->on_pre_playback_currenttime(curtime);
}

void sirius::app::client::proxy::on_playback_currenttime(int32_t curtime)
{
	if (_handler)
		_handler->on_playback_currenttime(curtime);
}

void sirius::app::client::proxy::on_post_playback_currenttime(int32_t curtime)
{
	if (_handler)
		_handler->on_post_playback_currenttime(curtime);
}

void sirius::app::client::proxy::on_pre_playback_currentrate(float currate)
{
	if (_handler)
		_handler->on_pre_playback_currentrate(currate);
}

void sirius::app::client::proxy::on_playback_currentrate(float currate)
{
	if (_handler)
		_handler->on_playback_currentrate(currate);
}

void sirius::app::client::proxy::on_post_playback_currentrate(float currate)
{
	if (_handler)
		_handler->on_post_playback_currentrate(currate);
}
