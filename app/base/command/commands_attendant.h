		...
		namespace attendant
		{
			...
			public:
				...
				void execute(const char * dst, const char * src, int32_t command_id, uint8_t version, const char * msg, int32_t length, std::shared_ptr<sirius::library::net::sicp::session> session)
				{
					...
					//LOGGER::make_info_log(SLNS, "%s, %d attendant_to_app data=%s, dst=%s, src=%s", __FUNCTION__, __LINE__, msg, dst, src);
				}
			};

			class sync_end2end_data_noti : public sirius::app::attendant::attendant_cmd
			{
			public:
				sync_end2end_data_noti(sirius::app::attendant::proxy * attendant)
					: sirius::app::attendant::attendant_cmd(attendant, CMD_SYNC_END2END_DATA_IND)
				{}
				...
				void execute(const char * dst, const char * src, int32_t command_id, uint8_t version, const char * msg, int32_t length, std::shared_ptr<sirius::library::net::sicp::session> session)
				{
					_attendant->sync_attendant_to_app_callback((uint8_t*)msg, length);
					//LOGGER::make_info_log(SLNS, "%s, %d attendant_to_app data=%s, dst=%s, src=%s", __FUNCTION__, __LINE__, msg, dst, src);
				}
			};
		};
