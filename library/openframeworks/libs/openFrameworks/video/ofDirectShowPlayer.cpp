...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
DEFINE_GUID(CLSID_CAP_CASP_SOURCE_FILTER,
	0xb7ae6843, 0xae99, 0x42a4, 0x95, 0x6d, 0xff, 0x9f, 0xab, 0x8e, 0xf5, 0xdf);

DEFINE_GUID(CLSID_FFDSHOW_DECODE_FILTER,
...
DEFINE_GUID(CLSID_CAP_VIDEO_DECODE_FILTER,
	0xc93156b, 0x1214, 0x4186, 0x95, 0xcd, 0x1b, 0xfb, 0x11, 0xf3, 0xf0, 0x53);

DEFINE_GUID(CLSID_Elecard_Color_Space_Converter2,
	0x5DB7637A, 0xF0DA, 0x4FC7, 0x9E, 0x45, 0x34, 0x1C, 0x0C, 0x83, 0x86, 0xB4);

DEFINE_GUID(CLSID_Video_Mixing_Renderer,
	0xB87BEB7B, 0x8D29, 0x423F, 0xAE, 0x4D, 0x65, 0x82, 0xC1, 0x01, 0x75, 0xAC);
//{ 0xB87BEB7B, 0x8D29, 0x423F, 0xAE, 0x4D, 0x65, 0x82, 0xC1, 0x01, 0x75, 0xAC};

DEFINE_GUID(CLSID_Video_Renderer,
...
class DirectShowVideo : public ISampleGrabberCB{
    ...
    void tearDown(){
		...
		{
			m_pColorSpaceConverter->Release();
		}
        if( m_pPosition ){
            m_pPosition->Release();
        }
	...
	{
		//::OutputDebugStringA("Enter SampleCB\n");
        ...
        if(hr == S_OK)
		{
			...
			//long currentBufferLength = pixels.getTotalBytes();
			...
			//		OutputDebugStringA("ERROR: SampleCB() - buffer sizes do not match \n");
			//}
        }

		//::OutputDebugStringA("Leave SampleCB\n");
        return S_OK;
    }
    ...
    // Create the Filter Graph Manager and query for interfaces.
		...
		hr = m_pGraph->QueryInterface(IID_IMediaFilter, (void **)&m_pMediaFilter);
		if (FAILED(hr))
		{
			tearDown();
			return false;
		...
		HRESULT hr = CoCreateInstance(CLSID_CAP_CASP_SOURCE_FILTER, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**)&m_pSourceFile);
		if (FAILED(hr))
		{
		...
		hr = m_pSourceFile->QueryInterface(IID_IFileSourceFilter, (void **)&fs);
		if (fs)
		{
			wchar_t url[MAX_PATH] = { 0 };
			wcsncpy_s(url, filePathW.c_str(), filePathW.size());
		...
		hr = m_pGraph->AddFilter(m_pSourceFile, L"source");
		if (FAILED(hr))
		{
			printf("unable to AddSourceFilter\n");
			tearDown();
		...
		hr = CoCreateInstance(CLSID_CAP_VIDEO_DECODE_FILTER, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**)&m_pVideoDecoder);
		if (FAILED(hr))
		{
			printf("unable to Create Video Decoder\n");
			tearDown();
			return false;
		}
		hr = m_pGraph->AddFilter(m_pVideoDecoder, L"video_decoder");
		if (FAILED(hr))
		{
			printf("unable to Add Video Decoder\n");
			tearDown();
			return false;
		}

		hr = ConnectFilters(m_pGraph, m_pSourceFile, m_pVideoDecoder);
		if (FAILED(hr))
		{
			m_pSourceFile->Stop();
			printf("unable to ConnectFilters(m_pGraph, m_pSourceFile, m_pVideoDecoder)\n");
			tearDown();
			return false;
		}

		/*
		hr = CoCreateInstance(CLSID_Elecard_Color_Space_Converter2, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**)&m_pColorSpaceConverter);
		if (FAILED(hr))
		{
			printf("unable to Create Color Space Converter\n");
			tearDown();
			return false;
		}
		hr = m_pGraph->AddFilter(m_pColorSpaceConverter, L"colorspace_converter");
		if (FAILED(hr))
		{
			printf("unable to Add Color Space Converter\n");
			tearDown();
			return false;
		}

		hr = ConnectFilters(m_pGraph, m_pVideoDecoder, m_pColorSpaceConverter);
		if (FAILED(hr))
		{
			printf("unable to ConnectFilters(m_pGraph, m_pVideoDecoder, m_pColorSpaceConverter)\n");
			tearDown();
			return false;
		}
		*/
		...
		hr = ConnectFilters(m_pGraph, m_pVideoDecoder, m_pGrabberF);
		if (FAILED(hr))
		{
			printf("unable to ConnectFilters(m_pGraph, m_pColorSpaceConverter, m_pGrabberF)\n");
			tearDown(); 
			return false;
        }
        ...
        if (SUCCEEDED(hr)){
			...
			*/
			...
			hr = CoCreateInstance(CLSID_Video_Renderer, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**)(&m_pVideoRenderer));
			if (FAILED(hr)) 
			{
				printf("null renderer error\n");
				tearDown();
			...
			hr = m_pGraph->AddFilter(m_pVideoRenderer, L"video_renderer");
			if (FAILED(hr)) 
			{
				printf("unable to add null renderer\n");
				tearDown();
			...
			hr = ConnectFilters(m_pGraph, m_pGrabberF, m_pVideoRenderer);
			if (FAILED(hr))
			{
				printf("unable to ConnectFilters(m_pGraph, m_pGrabberF, m_pNullRenderer)\n");
				tearDown();
			...
			{            
				...
				OutputDebugStringA("failed to connect the null renderer\n");
                tearDown();
                return false;
            }
			...
			{
                tearDown();
				OutputDebugStringA("Error occured while playing or pausing or opening the file\n");
    ...
    void update()
	{
        if( bVideoOpened )
		{
			...
			HRESULT hr = m_pEvent->GetEvent(&eventCode, &ptrParam1, &ptrParam2, 0);
            while (hr == S_OK)
			{
                if (eventCode == EC_COMPLETE )
				{
					...
					else
					{
						...
						::OutputDebugStringA("movie end reached!\n");
                        stop();
                        updatePlayState(); 
                    }
                }
				...
				hr = m_pEvent->GetEvent(&eventCode, &ptrParam1, &ptrParam2, 0);
            }
        }
    }
    ...
    void processPixels(ofPixels & src, ofPixels & dst){
        ...
        if(needsRBSwap(src.getPixelFormat(), dst.getPixelFormat())){
			...
			else if (src.getPixelFormat() == OF_PIXELS_BGRA) 
			{
				...
				dst.allocate(src.getWidth(), src.getHeight(), OF_PIXELS_RGBA);
				auto dstLine = dst.getLines().begin();
				auto srcLine = src.getLines().begin();
				auto endLine = dst.getLines().end();
				for (; dstLine != endLine; dstLine++, srcLine++)
				{
					auto dstPixel = dstLine.getPixels().begin();
	...
	{
		::OutputDebugStringA("updatePlayState\n");
        if( bVideoOpened )
		{
            FILTER_STATE fs;
            ...
            if(hr==S_OK)
			{
                if( fs == State_Running )
				{
					::OutputDebugStringA("Filter State is State_Running\n");
                    bPlaying = true; 
                    bPaused = false;
                }
                else if( fs == State_Paused )
				{
					::OutputDebugStringA("Filter State is State_Paused\n");
                    bPlaying = false;
                    bPaused = true;
                }
				else if( fs == State_Stopped )
				{
					::OutputDebugStringA("Filter State is State_Stopped\n");
                    bPlaying = false;
                    bPaused = false;
                }
            }
	...
	{
		//if (bPlaying)
		//	::OutputDebugStringA("bPlaying is true\n");
		//else 
		//	::OutputDebugStringA("bPlaying is false\n");
		return bPlaying;
    }
    ...
    ofPixels & getPixels()
	{
        if(bVideoOpened && bNewPixels){
			...
			case OF_PIXELS_BGRA:
				...
				//::OutputDebugStringA("After OF_PIXELS_BGRA\n");
				break;
			}
