#ifndef _IOCP_UDP_CLIENT_H_
#define _IOCP_UDP_CLIENT_H_
		...
		namespace net
		{
			namespace iocp
			{
				namespace udp
				{
					...
					{
					public:
						client(sirius::library::net::client * processor, int32_t mtu, int32_t so_rcvbuf_size, int32_t so_sndbuf_size, int32_t recv_buffer_size, bool dynamic_alloc = false, bool multicast = false);
						virtual ~client(void);

						std::shared_ptr<sirius::library::net::session> connect(const char * address, int32_t port_number, int32_t io_thread_pool_count);
